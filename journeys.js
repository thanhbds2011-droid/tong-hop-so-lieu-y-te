    ...
    return `<article class="journey-card">
      <div class="journey-card-main">
        ...
        <div class="journey-location">
          <span class="journey-location-label">${iconSvg('pin','journey-inline-icon')}Hiện tại</span>
      ...
      <div class="journey-card-actions">
        <button class="small-btn btn-soft journey-action" data-kind="view" data-id="${esc(item.id)}" type="button">${iconSvg('eye')}<span>Xem hành trình</span></button>
        ${canWrite ? `<button class="small-btn btn-soft journey-action" data-kind="update" data-id="${esc(item.id)}" type="button">${iconSvg('edit')}<span>Cập nhật</span></button>
        <button class="small-btn btn-primary journey-action" data-kind="return" data-id="${esc(item.id)}" type="button">${iconSvg('home')}<span>Đã về Trung tâm</span></button>` : ''}
      </div>
    </article>`;
  }).join('');
}
  ...
  box.innerHTML = rows.map((item) => {
    if (item.kind === 'CENTER_DEATH') {
      return `<article class="journey-history-card is-center-death">
        ...
        <div class="journey-history-actions"><button class="small-btn btn-soft journey-history-action" data-kind="center-death-view" data-id="${esc(item.id)}" type="button">${iconSvg('eye')}<span>Xem</span></button>${canEdit() ? `<button class="small-btn btn-soft journey-history-action" data-kind="center-death-edit" data-id="${esc(item.id)}" type="button">${iconSvg('edit')}<span>Sửa</span></button>` : ''}${(isOwner() || (state.permission && state.permission.role === 'admin')) ? `<button class="small-btn btn-danger journey-history-action" data-kind="center-death-delete" data-id="${esc(item.id)}" type="button"><span>Xóa</span></button>` : ''}</div>
      </article>`;
    }
